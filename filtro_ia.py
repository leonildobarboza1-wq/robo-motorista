...
def processar_vaga_com_gemini(vaga, data_postagem):
    prompt = f"Escreva uma matéria detalhada de 300 palavras sobre a vaga: {vaga['titulo']}. Retorne JSON com 'titulo_otimizado' e 'conteudo_html' (use <p> e <ul>, sem <img>)."
    try:
        dados = chamar_api_gemini_com_json(prompt)
        # Montagem do HTML com o link da fonte inserido corretamente
        ...
        return {"titulo_otimizado": vaga['titulo'], "conteudo_html": f"<p>Vaga disponível em {vaga['fonte']}. <a href='{vaga['link']}'>Clique aqui para ver</a>.</p>"}
...
def formatar_noticia_com_gemini(noticia, data_postagem):
    ...
    try:
        ...
        return {"titulo_otimizado": noticia['titulo'], "conteudo_html": f"<p>Leia mais em: <a href='{noticia['link']}'>{noticia['fonte_original']}</a></p>"}
